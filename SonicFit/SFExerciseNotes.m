...
- (id)init {
    ...
    if (self) {
        ...
        self.exerciseNotesArray3 = [self getExerciseLevel3];
    }
...
{
    NSArray *notes1 = [NSArray arrayWithObjects:@"FA", @"RE", @"DO", nil];
    NSArray *notes2 = [NSArray arrayWithObjects:@"DO", @"RE", @"MI", nil];
    NSArray *notes3 = [NSArray arrayWithObjects:@"MI", @"SOL", @"LA", nil];
    NSArray *notes4 = [NSArray arrayWithObjects:@"MI", @"SOL", @"MI", nil];
    NSArray *notes5 = [NSArray arrayWithObjects:@"SOL", @"LA", @"SOL", nil];
    NSArray *notes6 = [NSArray arrayWithObjects:@"SOL", @"DO", @"SOL", nil];
    NSArray *notes7 = [NSArray arrayWithObjects:@"RE", @"FA", @"FA", nil];
    NSArray *notes8 = [NSArray arrayWithObjects:@"FA", @"MI", @"SOL", nil];
...
- (NSArray *)getNextExerciseNotesForLevel:(int)level;
{
    if (level == 1)
    {
        int i = arc4random() % self.exerciseNotesArray1.count;
        return [self.exerciseNotesArray1 objectAtIndex:i];
    }
    else if (level == 2)
    {
        int i = arc4random() % self.exerciseNotesArray2.count;
        return [self.exerciseNotesArray2 objectAtIndex:i];
    }
    else if (level == 3)
    {
        int i = arc4random() % self.exerciseNotesArray3.count;
        return [self.exerciseNotesArray3 objectAtIndex:i];
    }
    return nil;
}
